...
class RobotController(Node):
    def __init__(self):
        ...
        # )
        # self.camera_matrix = cv2_file.getNode("K").mat()
        # self.dist_coeffs = cv2_file.getNode("D").mat()
        
        # self.rotation_matrix = cv2_file.getNode("R").mat()
        # cv2_file.release()     
        ...
        # Camera Extrinsic Parameters (Transform from camera to world)
        self.camera_to_world_translation = np.array([0.0, 0.0, 0.0])  # x, y, z
        
        self.camera_to_world_rotation = np.eye(3)  # 3x3 rotation matrix
    ...
    def camera_info_callback(self, msg):
        ...
        self.rotation_matrix = self.projection_matrix[:, :3]
    ...
    def set_camera_extrinsics(self, translation):
        """
        Set camera extrinsic parameters for world coordinate transformation
        
        :param translation: 3D translation vector [x, y, z]
        :param rotation_matrix: 3x3 rotation matrix from camera to world frame
        """
        self.camera_to_world_translation = np.array(translation)
        self.camera_to_world_rotation = np.array(self.rotation_matrix)
        self.get_logger().info(f'Camera extrinsics set - Translation: {translation}')

    def camera_to_world_coords(self, camera_point, depth=1.0):
        """
        Convert camera frame coordinates to world coordinates
        
        :param camera_point: 2D point in camera pixel coordinates
        :param depth: Depth from camera (default 1.0 meter)
        :return: 3D world coordinates
        """
        if (self.camera_matrix is None or 
            self.dist_coeffs is None or 
            self.camera_to_world_rotation is None):
            self.get_logger().warn('Camera calibration not fully configured')
            return None

        try:
            # Undistort point
            undistorted_points = cv2.undistortPoints(
                np.array([camera_point], dtype=np.float32), 
                self.camera_matrix, 
                self.dist_coeffs
            )

            # Convert 2D pixel to 3D camera frame coordinates
            # Assumes a default depth of 1 meter
            camera_coords = np.array([
                undistorted_points[0][0][0] * depth, 
                undistorted_points[0][0][1] * depth, 
                depth
            ])

            # Transform from camera frame to world frame
            world_coords = (
                self.camera_to_world_rotation @ camera_coords + 
                self.camera_to_world_translation
            )

            return (world_coords[0], world_coords[1])  # Return x, y coordinates
        
        except Exception as e:
            self.get_logger().error(f'Coordinate transformation error: {str(e)}')
            return None

    def analyze_markers(self, corners, ids, tvecs=None):
        """Analyze markers and convert to world coordinates"""
        try:
            for i, marker_id in enumerate(ids.flatten()):
                marker_corners = corners[i][0]
                center = np.mean(marker_corners, axis=0)
                
                # Convert camera point to world coordinates
                world_point = self.camera_to_world_coords(center)
                
                if marker_id == self.robot_marker_id:
                    # Update robot position from ArUco marker
                    self.robot_position = world_point if world_point else self.robot_position
                else:
                    # Check if goal is already known
                    goal_exists = any(goal.goal_id == marker_id for goal in self.all_goals)
                    
                    if not goal_exists and world_point:
                        # Create new goal with both camera and world coordinates
                        new_goal = Goal(
                            goal_id=marker_id,
                            camera_pos=(center[0], center[1]),
                            world_pos=world_point
                        )
                        self.all_goals.append(new_goal)
                        self.unvisited_goals.append(new_goal)
                        self.get_logger().info(
                            f'New goal discovered: ID {marker_id}, '
                            f'Camera Pos: {new_goal.camera_pos}, '
                            f'World Pos: {new_goal.world_pos}'
                        )
    ...
    def select_closest_goal(self) -> Goal:
        ...
        closest_goal = min(
            ...
            key=lambda goal: math.hypot(
                self.robot_position[0] - goal.world_pos[0],
                self.robot_position[1] - goal.world_pos[1]
            )
        )
        return closest_goal
    ...
    def control_loop(self):
        ...
        if not self.current_goal and self.unvisited_goals:
            ...
            self.get_logger().info(
                ...
                f'Position {self.current_goal.world_pos}'
            )
        ...
        # Get target position
        target_x, target_y = self.current_goal.world_pos
        ...
        try:
            with open(self.goals_csv_path, 'w', newline='') as csvfile:
                ...
                for goal in self.all_goals:
                    csvwriter.writerow([
                        goal.goal_id, 
                        goal.camera_pos[0], goal.camera_pos[1],
                        goal.world_pos[0], goal.world_pos[1], 
                        goal.achieved
                    ])
...
def main():
    ...
    controller.set_camera_extrinsics(
        translation=[1.4, 0, 2.1],  # x, y, z offset of camera)
    )
