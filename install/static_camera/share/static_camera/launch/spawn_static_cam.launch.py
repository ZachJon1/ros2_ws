...
for pose_element, default_value in default_pose.items():
...
def generate_launch_description():
    ...
    camera_bridge = Node(
        ...
        remappings=[
            (['/world/', world_name, '/model/', 
              camera_name, '/link/link/sensor/camera/image'], [camera_name, '/image_raw']),
    ...
    spawn_camera = Node(
        ...
        parameters=[{
            'robot_description': Command(['xacro ', 'update_rate:=', update_rate, ' image_width:=', image_width, ' image_height:=', image_height, ' camera_name:=', camera_name, ' ', PathJoinSubstitution([static_camera_dir, "models", "camera.sdf.xacro"])])}],
        output='screen'
    )
    ...
    ld.add_action(camera_subscription)
    return ld
